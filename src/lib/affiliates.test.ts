...
const AMAZON: AffiliateRule = { host: 'amazon.ca', mode: 'param', params: { tag: 'savedd-21' } };
...
describe('applyAffiliateRules', () => {
  it('Amazon: appends ?tag= to a product URL', () => {
    ...
    expect(out).toBe('https://www.amazon.ca/dp/B0EXAMPLE?tag=savedd-21');
  });
  ...
  it('Amazon: replaces an existing (scraped) tag with ours', () => {
    ...
    expect(out).toBe('https://amazon.ca/dp/B0EXAMPLE?tag=savedd-21');
  });
  ...
  it('subdomains match (www., smile., etc.)', () => {
    expect(applyAffiliateRules('https://smile.amazon.ca/x', [AMAZON])).toContain('tag=savedd-21');
...
describe('schema back-compat', () => {
  ...
  it('old single param/value rules parse as one-entry params', () => {
    const event = fakeEvent(JSON.stringify({
      version: 1,
      rules: [{ host: 'amazon.ca', param: 'tag', value: 'savedd-21' }],
    }));
    const rules = parseAffiliateRules(event, owner);
    expect(rules).toEqual([{ host: 'amazon.ca', mode: 'param', params: { tag: 'savedd-21' }, target: undefined }]);
  });
  ...
  it('single-param rules serialize with legacy param/value fields too', () => {
    ...
    expect(written.rules[0].params).toEqual({ tag: 'savedd-21' });
    expect(written.rules[0].param).toBe('tag');
    expect(written.rules[0].value).toBe('savedd-21');
  });
